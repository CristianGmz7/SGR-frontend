  ...
  return (
    <>
      <section className="border rounded-lg p-6 mb-6 shadow-md bg-blue-100">
        <h2 className="text-lg font-bold text-blue-800 mb-4">Reserva: {reservation.reservationId}</h2>
        ...
        <div className="flex justify-between text-blue-600 mb-6">
          <p>Total dias: {calculateDays(reservation.reservationStartDate, reservation.reservationFinishDate)}</p>
        </div>
        ...
        <h3 className="text-md font-bold mt-4 text-blue-700 mb-2">Servicios Adicionales</h3>
        {
          ...
          ? (
            reservation.reservationAdditionalServicesInfoList.map((aS) => (
              <AdditioServReservationCard key={aS.additionalServiceId} aS={aS}/>
            ))

          )
          : (
            <div className="border rounded-lg p-4 shadow-md bg-white hover:bg-blue-50 transition-colors mb-2">
              <p className="text-blue-700">No hay servicios adicionales para esta reserva</p>
            </div>
          )
        }

        <p className="font-bold mt-4 text-blue-800">Total Reserva = ${reservation.reservationPrice}</p>
